<template>
    <div id="order-container">
        <table>
            ...
            <tbody>
            <tr v-for="item in order_data " :key="item.order_goods_id">
                <td>
                    <img :src="item.order_goods_img" class="order_img"/>
                    <div class="carslist_name">
                        {{item.order_goods_name}}<br>
                        <div style="margin-top: 10px">尺码：M</div>
                    </div>
                </td>
                <td>￥{{item.order_goods_price}}</td>
                <td>
                    <div class="goods_details_num" >
                        <span @click="handleCountAdd(item)">+</span><span v-model="item.order_goods_num">{{item.order_goods_num}}</span><span @click="handleCountLess(item)">-</span>
                    </div>
                </td>
                <td>￥{{item.order_goods_num*item.order_goods_price}}</td>
            </tr>
            </tbody>
        </table>
        ...
        <div class="total_pay">
            ...
            <div class="to_pay">
                <router-link to="/other_container/goods_pay"><button>立即结算</button></router-link>
            </div>
        </div>
...
export default {
    ...
    data(){
        return{
            ...
            order_data:[
                {
                    order_goods_id:1,
                    order_goods_img:'../src/image/goods1.jpg',
                    order_goods_name: '香影新款很仙的印花裙子夏款流行荷叶边裙超仙雪纺连衣裙',
                    order_goods_price:'10',
                    order_goods_num:1
                },
                {
                    order_goods_id:2,
                    order_goods_img:'../src/image/goods1.jpg',
                    order_goods_name: '香影新款很仙的印花裙子夏款流行',
                    order_goods_price:'10',
                    order_goods_num:1
                }
            ] ,
    ...
    computed:{
        ...
        selectedMoney()
        {
            ...
            this.order_data.forEach((item, index) => {
                sum += parseInt(this.order_data[index].order_goods_price) * this.order_data[index].order_goods_num;
            });
            this.all_money = sum;
    ...
    methods:{
        ...
        //数量++
        handleCountAdd(item){
            console.log(item.order_goods_num);
            if(item.order_goods_num <5) item.order_goods_num++;
            else {item.order_goods_num =5; alert("限购5件");}
        },
        //数量--
        handleCountLess(item){
            if(item.order_goods_num >0) item.order_goods_num--;
            else item.order_goods_num = 0;
        },
        change(e) {
